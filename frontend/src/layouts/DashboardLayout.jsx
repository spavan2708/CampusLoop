import { CalendarCheck2, CalendarPlus, Compass, House, ListChecks, LogOut, Menu, UserRound, X } from 'lucide-react'
  ...
  return (
    <div className="dashboard-shell">
      <header className="dashboard-header">
        ...
        {user.role === 'organizer' && <nav className={`dashboard-nav ${menuOpen ? 'nav-open' : ''}`} aria-label="Organizer navigation">
          <NavLink to="/organizer" end onClick={() => setMenuOpen(false)}><House /> Dashboard</NavLink>
          <NavLink to="/organizer/events" onClick={() => setMenuOpen(false)}><ListChecks /> Manage Events</NavLink>
          <NavLink to="/organizer/events/new" onClick={() => setMenuOpen(false)}><CalendarPlus /> Create Event</NavLink>
          <NavLink to="/organizer/profile" onClick={() => setMenuOpen(false)}><UserRound /> Profile</NavLink>
        </nav>}

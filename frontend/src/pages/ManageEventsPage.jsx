...
export default function ManageEventsPage() {
  ...
  return <>
    <main className="dashboard-main">
      ...
      <div className="manage-toolbar"><label className="search-field"><Search /><span className="sr-only">Search owned events</span><input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search your events" /></label><div className="status-tabs" aria-label="Filter by status">{Object.keys(statusLabels).map((option) => <button className={status === option ? 'active' : ''} type="button" onClick={() => setStatus(option)} key={option}>{statusLabels[option]}</button>)}</div></div>

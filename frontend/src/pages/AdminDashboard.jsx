import { Search, ShieldCheck, Sparkles, UsersRound, Trash } from 'lucide-react'
...
export default function AdminDashboard() {
  ...
  async function confirmAction(reason) {
    ...
    try {
      ...
      else if (current.kind === 'delete-club') {
        await deleteClub(current.club.id)
        setDialog(null); await refresh(); setMessage({ type: 'success', text: 'Club permanently deleted.' })
        return
      }
  ...
  return <><main className="dashboard-main admin-page">
    ...
    <section className="admin-section"><div className="section-title-row"><div><span className="dashboard-kicker">Access control</span><h2>Club accounts</h2></div><Link className="text-link" to="/admin/clubs/new">Add club →</Link></div>{clubs.length ? <div className="admin-table">{clubs.map((club) => <article key={club.id}><div><strong>{club.name}</strong><span>{club.category} · {club.contact_email}</span><StatusBadge value={club.is_active ? 'active' : 'inactive'} /></div><div className="row-actions"><button className={club.is_active ? 'button button-danger button-small' : 'button button-secondary button-small'} onClick={() => setDialog({ kind: 'club', club })}>{club.is_active ? 'Deactivate login' : 'Reactivate login'}</button><button className="button button-secondary button-small" onClick={() => setDialog({ kind: 'delete-club', club })}><Trash /> Delete</button></div></article>)}</div> : <EmptyState title="No club accounts" message="Create the first club login to begin." />}</section>
  ...
  </main><ConfirmDialog open={Boolean(dialog)} title={dialog?.kind === 'club' ? `${dialog.club.is_active ? 'Deactivate' : 'Reactivate'} club login?` : dialog?.kind === 'delete-club' ? `Delete ${dialog.club.name}?` : `${dialog?.action?.replace('-', ' ') || ''} event?`} description={dialog?.kind === 'club' ? 'This changes the club login and its ability to manage events.' : dialog?.kind === 'delete-club' ? 'This will permanently delete the club and all its events, registrations, and saved events. This action cannot be undone.' : 'This moderation decision is recorded and immediately affects the event workflow.'} reasonLabel={['reject', 'request-changes', 'cancel'].includes(dialog?.action) ? 'Reason (required)' : null} confirmLabel={dialog?.kind === 'delete-club' ? 'Delete club' : 'Confirm action'} tone={['reject', 'cancel'].includes(dialog?.action) || (dialog?.kind === 'club' && dialog.club.is_active) || dialog?.kind === 'delete-club' ? 'danger' : 'primary'} busy={busy} onCancel={() => setDialog(null)} onConfirm={confirmAction} /></>
}
